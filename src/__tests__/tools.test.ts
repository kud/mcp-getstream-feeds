...
describe("getFeedFollowing", () => {
  it("returns following list on success", async () => {
    const results = [{ feed_id: "streams_updated:streams_updated_all" }]
    mockFollowing.mockResolvedValue({ results })
    const result = await getFeedFollowing({
      feed_slug: "user_mobile_push",
      feed_id: "InsightsProfile_366401",
    })
    expect(text(result)).toContain("streams_updated_all")
    expect(mockFeed).toHaveBeenCalledWith(
      "user_mobile_push",
      "InsightsProfile_366401",
    )
  ...
  it("returns error when SDK throws", async () => {
    ...
    expect(text(result)).toContain("user_mobile_push:abc")
  })
})
...
describe("getFeedFollowers", () => {
  it("returns followers list on success", async () => {
    const results = [{ feed_id: "user_mobile_push:InsightsProfile_366401" }]
    mockFollowers.mockResolvedValue({ results })
    const result = await getFeedFollowers({
      feed_slug: "streams_updated",
      feed_id: "streams_updated_all",
    })
    expect(text(result)).toContain("InsightsProfile_366401")
    expect(mockFeed).toHaveBeenCalledWith(
      "streams_updated",
      "streams_updated_all",
    )
...
describe("followFeed", () => {
  it("creates follow relationship when confirm is true", async () => {
    ...
    const result = await followFeed({
      source_feed_slug: "user_mobile_push",
      source_feed_id: "InsightsProfile_366401",
      target_feed_slug: "streams_updated",
      target_feed_id: "streams_updated_all",
      confirm: true,
    })
